...
@auth_bp.route(URL.BASE, methods=["GET", "POST"])
def login():
    ...
    if loginForm.validate_on_submit():
        ...
        if user is None:
            flash("This user does not exists. Please sign up", "warning")
        ...
        if not user.verified:
            flash("Please check your inbox and validate the account")
            return redirect(url_for("auth_bp.login"))
        ...
        if user and user.check_password(password=loginForm.password.data):
            ...
            user_dict = {
                key: user.__dict__[key] for key in ["id", "first_name", "last_name"]
            }
            db.session.commit()
...
@auth_bp.route(URL.SIGNUP, methods=["GET", "POST"])
def signup():
    """
    User sign-up page.
...
@auth_bp.route(URL.VERIFY_EMAIL)
